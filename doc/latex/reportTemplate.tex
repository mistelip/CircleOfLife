...
%Question 1: Is it possible to create a stable system that closely resembles nature? Yes
...
%Qustion 3: Does it remain stable if lions overfeed?
...
%Question 4: Do the two models, the Lotka-Volterra model and our model match in the sense, that they show the same simulation results? YES, except when removing lions
...
\hline 
\addtocounter{attCounter}{1}
\arabic{attCounter}. DeathProb [0,1]& The probability of dying by age at the end of a time-step. Other death causes are discussed in section \ref{sec:DeathCauses} \\ 
\hline 
\addtocounter{attCounter}{1}
...
Running multiple simulations and adjusting the parameters and the initial map there were several dependencies we discovered. Additionally we compared the population results obtained form the simulation with the Lotka-Volterra population curves for three species, \cite{lotkaVolterraThreeSpecies}. 
